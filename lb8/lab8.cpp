...
#include <iostream>
#include <string>   // to generate uniform integer distributions
#include "Node.h"
using namespace std;
...
void insert( Node<T>* &headPtr , const unsigned& new_node_value )
{
	...
	if (headPtr== nullptr ||  headPtr->getItem() >= new_node_value)
	{
		...
		new_node->setPrev(headPtr->getPrev());
	}
		headPtr = new_node;
	} 
	else 
...
int removeEvens( Node<T>* &headPtr)
{
	...
	while(currentptr!=NULL) 
    {
    	if(prev->getItem()%2 == 0)
    	{
          ...
          prev->setPrev(prev->getNext()->getPrev());
    	...
    	{	
    		...
    		prev->setPrev(currentptr->getPrev());
			free(temp);
